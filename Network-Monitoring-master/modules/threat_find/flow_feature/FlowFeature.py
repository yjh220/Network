class FlowFeature:
    ...
    def returnFeature(self):
        return [
            self.flowId, self.srcIP, self.srcPort, self.dstIP, self.dstPort, self.protocol, "", self.flowDurationMS,

            self.fwdPktNum, self.bwdPktNum, self.fwdPldByteSum, self.bwdPldByteSum,

            self.fwdPldByteMax, self.fwdPldByteMin, self.fwdPldByteMean, self.fwdPldByteStd,
            self.bwdPldByteMax, self.bwdPldByteMin, self.bwdPldByteMean, self.bwdPldByteStd,

            self.flowPldBytesMS * 1000, self.flowPktsS, self.flowIatMean, self.flowIatStd,
            self.flowIatMax, self.flowIatMin,

            self.fwdIatMax + self.fwdIatMin + self.fwdIatMean * self.fwdPktNum, self.fwdIatMean, self.fwdIatStd,
            self.fwdIatMax, self.fwdIatMin,

            self.bwdIatMax + self.bwdIatMin + self.bwdIatMean * self.bwdPktNum, self.bwdIatMean, self.bwdIatStd,
            self.bwdIatMax, self.bwdIatMin,

            self.fwdPSHcnt, self.bwdPSHcnt, self.fwdURGcnt, self.bwdURGcnt,

            self.fwdHeadByteMean * self.fwdPktNum, self.bwdHeadByteMean * self.bwdPktNum,

            self.fwdPktsS, self.bwdPktsS,

            self.flowPldByteMin, self.flowPldByteMax, self.flowPldByteMean, self.flowPldByteStd, 0,  # Variance 可后续补充

            self.FINcnt, self.SYNcnt, self.RSTcnt, self.PSHcnt, self.ACKcnt, self.URGcnt,
            self.CWRcnt, self.ECEcnt, self.bytesRatio, self.flowPldByteMean,
            self.fwdPldByteMean, self.bwdPldByteMean,

            self.fwdHeadByteMean * self.fwdPktNum,

            self.fwdAvgBytesPerBulk, self.fwdAvgPktsPerBulk, self.fwdAvgBulkS,
            self.bwdAvgBytesPerBulk, self.bwdAvgPktsPerBulk, self.bwdAvgBulkS,

            self.subFlowFwdPkts, self.subFlowFwdPldBytes, self.subFlowBwdPkts, self.subFlowBwdPldBytes,

            self.fwdInitWinBytes, self.bwdInitWinBytes, self.fwdPktsWithPayload, 0,  # min_seg_size_forward 可后续补充

            self.flowActMean, self.flowActStd, self.flowActMax, self.flowActMin,
            self.flowIdleMean, self.flowIdleStd, self.flowIdleMax, self.flowIdleMin,
