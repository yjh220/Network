...
class FlowGenerator:
    ...
    def dumpFeatureToCSV(self,filename="test.csv"):
        # 将流量统计特征保存到CSV文件
        with open(filename, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)

            # 写入特征名称
            columns_name = get_train_header()
    ...
    def get_features_dict(self):
        """获取带列名的字典格式数据"""
        columns = get_train_header()
        return [
            dict(zip(columns, flow.generateFlowFeatures()))
            for flow in self.finishedFlows
        ]
